class LeadFinderExtension {
  ...
  async handleSearch() {
    ...
    const max = Number.parseInt(document.getElementById("max-results").value) || 20;
    ...
    if (!q || !loc) { this.showError("Please enter both business type and location"); return; }
    if (max < 1 || max > cfg.maxResults) { this.showError(`Number of results must be between 1 and ${cfg.maxResults} for your ${cfg.name}`); return; }
    if (wf !== "all" && !cfg.hasAdvancedFiltering) { this.showError("Advanced filtering requires Pro or Enterprise plan. Please upgrade to use this feature."); return; }
    this.showSearchProgress(true);
    try {
      const res = await fetch(`${this.apiBase}/api/search`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${this.token}` }, body: JSON.stringify({ query: q, location: loc, maxResults: max, websiteFilter: cfg.hasAdvancedFiltering ? wf : "all" }) });
      const data = await res.json();
      if (!res.ok) { if (data.error?.includes('quota') || data.error?.includes('limit')) { this.showQuotaExceededMessage(tier); } else { throw new Error(data.error || "Search failed"); } return; }
      this.displayResults(data.results, wf, tier); this.updateQuotaInfo(); this.showSuccess(`Found ${data.results.length} leads using ${cfg.name}!`);
    } catch (e) { this.showError(e.message); } finally { this.showSearchProgress(false); }
  }
  ...
  displayResults(results, wf = "all", tier = "free") {
    ...
    if (results.length === 0) {
      ...
      let filterInfo = "";
      if (wf === "no-website" && cfg.hasAdvancedFiltering) { const n = results.filter(r => !r.website).length; filterInfo = `<div class=\"filter-count\">🔍 Advanced Filter: Showing ${n} businesses without websites</div>`; }
      else if (wf === "has-website" && cfg.hasAdvancedFiltering) { const n = results.filter(r => r.website).length; filterInfo = `<div class=\"filter-count\">🔍 Advanced Filter: Showing ${n} businesses with websites</div>`; }
      const tierBadge = `<div class=\"tier-badge tier-${tier}\">Results powered by ${cfg.name}</div>`;
      container.innerHTML = tierBadge + filterInfo + results.map(r => `<div class=\"result-item tier-${tier}\"><div class=\"result-name\">${r.name}${!r.website?'<span class=\"no-website-badge\">No Website</span>':''}</div><div class=\"result-details\">${r.formatted_address}<br>${r.formatted_phone_number?`📞 ${r.formatted_phone_number}<br>`:''}${r.website?`🌐 <a href=\"${r.website}\" target=\"_blank\">${r.website}</a><br>`:''}${r.rating?`⭐ ${r.rating} (${r.user_ratings_total||0} reviews)`:''}</div></div>`).join('');
    }
