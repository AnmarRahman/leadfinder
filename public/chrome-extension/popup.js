class LeadFinderExtension {
  ...
  updateUIForSubscriptionTier() {
    ...
    const cfg = this.tierFeatures[tier];
    this.updateSubscriptionDisplay(cfg, tier);
    this.updateMaxResultsLimit(cfg);
    this.updateFilterAccess(cfg);
    this.updateExportFeatures(cfg);
  }
  updateSubscriptionDisplay(cfg, tier) {
    const el = document.getElementById("current-tier");
    const up = document.getElementById("upgrade-btn");
    el.textContent = cfg.name;
    if (tier === "enterprise") up.style.display = "none";
    else {
  ...
  updateMaxResultsLimit(cfg) {
    const input = document.getElementById("max-results");
    const label = document.querySelector('label[for="max-results"]');
    input.max = cfg.maxResults;
    input.value = Math.min(input.value || cfg.maxResults, cfg.maxResults);
    label.textContent = `Number of Results (1-${cfg.maxResults})`;
    input.parentElement.setAttribute(
      "data-tier",
      this.userProfile?.subscription_tier || "free"
    );
  }
  updateFilterAccess(cfg) {
    const sel = document.getElementById("website-filter");
    const badge = document.getElementById("filter-pro-badge");
    const group = document.getElementById("website-filter-group");
    if (cfg.hasAdvancedFiltering) {
      sel.disabled = false;
      badge.classList.add("hidden");
      group.classList.remove("disabled-feature");
    } else {
      sel.disabled = true;
      sel.value = "all";
      badge.classList.remove("hidden");
      group.classList.add("disabled-feature");
    }
  }
  updateExportFeatures() {
    const btn = document.getElementById("export-btn");
    const tier = this.userProfile?.subscription_tier || "free";
    btn.textContent =
      tier === "enterprise"
        ? "Export All Leads to CSV (Enhanced)"
        : tier === "pro"
        ? "Export All Leads to CSV (Advanced)"
  ...
  async handleUpgrade() {
    ...
    await this.chrome.tabs.create({
      url: `${this.apiBase}/pricing?utm_source=extension&current_tier=${tier}&target_tier=${target}`,
    });
  }
  ...
  async handleSearch() {
    ...
    if (max < 1 || max > cfg.maxResults) {
      this.showError(
        `Number of results must be between 1 and ${cfg.maxResults} for your ${cfg.name}`
      );
      return;
    }
    if (wf !== "all" && !cfg.hasAdvancedFiltering) {
      this.showError(
    ...
    try {
      const res = await fetch(`${this.apiBase}/api/search`, {
        ...
        body: JSON.stringify({
          ...
          location: loc,
          maxResults: max,
          websiteFilter: cfg.hasAdvancedFiltering ? wf : "all",
        }),
      });
      const data = await res.json();
      if (!res.ok) {
        if (data.error?.includes("quota") || data.error?.includes("limit")) {
          this.showQuotaExceededMessage(tier);
        } else {
          throw new Error(data.error || "Search failed");
        }
        return;
      }
      this.displayResults(data.results, wf, tier);
      this.updateQuotaInfo();
      this.showSuccess(`Found ${data.results.length} leads using ${cfg.name}!`);
    } catch (e) {
      this.showError(e.message);
    } finally {
  ...
  showQuotaExceededMessage(tier) {
    const cfg = this.tierFeatures[tier];
    const next = tier === "free" ? "Pro" : "Enterprise";
    this.showError(
      `You've reached your ${cfg.name} limit of ${cfg.searches} searches. Upgrade to ${next} for more searches!`
    );
  }
  async handleExport() {
    const tier = this.userProfile?.subscription_tier || "free";
    this.showLoading(true);
    try {
      const endpoint = this.tierFeatures[tier].hasAdvancedFiltering
      ...
      a.href = url;
      const suffix = tier !== "free" ? `-${tier}` : "";
      a.download = `leads${suffix}-${
      ...
      this.showSuccess(
        `${
          this.tierFeatures[tier].hasAdvancedFiltering ? "Advanced" : "Basic"
        } leads export completed!`
      );
    } catch (e) {
      this.showError(e.message);
    } finally {
  ...
  displayResults(results, wf = "all", tier = "free") {
    ...
    if (results.length === 0) {
      ...
      let filterInfo = "";
      if (wf === "no-website" && cfg.hasAdvancedFiltering) {
        const n = results.filter((r) => !r.website).length;
        filterInfo = `<div class="filter-count">🔍 Advanced Filter: Showing ${n} businesses without websites</div>`;
      } else if (wf === "has-website" && cfg.hasAdvancedFiltering) {
        const n = results.filter((r) => r.website).length;
        filterInfo = `<div class="filter-count">🔍 Advanced Filter: Showing ${n} businesses with websites</div>`;
      }
      const tierBadge = `<div class="tier-badge tier-${tier}">Results powered by ${cfg.name}</div>`;
      container.innerHTML =
        tierBadge +
        filterInfo +
        results
          .map(
            (r) =>
              `<div class="result-item tier-${tier}"><div class="result-name">${
                r.name
              }${
                !r.website
                  ? '<span class="no-website-badge">No Website</span>'
                  : ""
              }</div><div class="result-details">${r.formatted_address}<br>${
                r.formatted_phone_number
                  ? `📞 ${r.formatted_phone_number}<br>`
                  : ""
              }${
                r.website
                ...
                r.rating
                  ? `⭐ ${r.rating} (${r.user_ratings_total || 0} reviews)`
                  : ""
              }</div></div>`
          )
          .join("");
  ...
  async updateQuotaInfo() {
    ...
    try {
      const res = await fetch(`${this.apiBase}/api/user/quota`, {
        headers: { Authorization: `Bearer ${this.token}` },
      });
      if (res.status === 401) {
        this.handleLogout();
      ...
      const data = await res.json();
      const monthly =
        typeof data.monthlyQuota === "number" ? data.monthlyQuota : null;
      const remaining =
        typeof data.remainingQuota === "number" ? data.remainingQuota : null;
      const used =
        typeof data.usedQuota === "number"
          ? data.usedQuota
          : monthly != null && remaining != null
          ? monthly - remaining
          : null;
      if (monthly != null && used != null) {
        quotaInfo.innerHTML = `<div>Searches: ${used} / ${monthly} used this month</div><div>Remaining: ${
          remaining != null ? remaining : "—"
        } searches</div><button id="quota-refresh" class="mini-upgrade-btn" style="margin-top:8px">Refresh</button>`;
      ...
      document
        .getElementById("quota-refresh")
        ?.addEventListener("click", () => this.updateQuotaInfo());
    } catch (e) {
  ...
  showSearchProgress(show) {
    const cont = document.getElementById("search-progress");
  ...
  animateProgress() {
    ...
    this.progressInterval = setInterval(() => {
      ...
      if (msgs[idx]) text.textContent = msgs[idx];
    }, 800);
  }
